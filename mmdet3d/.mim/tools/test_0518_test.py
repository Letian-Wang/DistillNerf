...
def main():
    ...
    if rank == 0:
        ...
        for one_output in outputs:
            ...
            input_depth_error_lst.append(one_output['input_depth_error'])
            target_depth_error_lst.append(one_output['target_depth_error'])
        print(f'psnr: {sum(psnr_lst) / len(psnr_lst)}') 
        print(f'ssim: {sum(ssim_lst) / len(ssim_lst)}')
        print("input_depth_error_lst: ", np.mean(np.stack(input_depth_error_lst), 0))
        print("target_depth_error_lst: ", np.mean(np.stack(target_depth_error_lst), 0))
        import pdb; pdb.set_trace()
